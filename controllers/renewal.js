var app = angular.module('renewal', ['globals','block-ui','bootstrap-notify','bootstrap-modal','account-module','notifications-module','lock-screen-module']);
...
app.service('crud',function($http,$compile,$timeout,bootstrapModal,blockUI,globalsService) {
	...
	scope.views.semesters = {
		...
		$http({
		  method: 'POST',
		  url: 'controllers/renewal.php?r=list'
	...
	this.view = function(scope,id) {
		...
		$http({
		  method: 'POST',
		  data: {id: id},
		  url: 'controllers/renewal.php?r=view'
		}).then(function mySucces(response) {
	...
	this.save = function(scope) {	
		...
		$http({
		  method: 'POST',
		  url: 'controllers/renewal.php?r='+scope.views.mode,
	...
	this.del = function(scope,id) {
		...
		function del(id) {
			...
			$http({
			  method: 'POST',
			  data: {id: [id]},
			  url: 'controllers/renewal.php?r=delete'
			}).then(function mySucces(response) {
...
app.controller('renewalCtrl',function($http,$timeout,$scope,crud,blockUI,bootstrapNotify,bootstrapModal) {
...
$scope.scholarshipSave = function() {
	...
	$http({
	  ...
	  url: 'controllers/renewal.php?r=update_scholarship'
	}).then(function mySucces(response) {
