var app = angular.module('grantees', ['globals','block-ui','bootstrap-notify','bootstrap-modal','account-module','notifications-module','lock-screen-module']);
...
app.service('crud',function($http,$compile,$timeout,bootstrapModal,blockUI,globalsService) {
	...
	blockUI.show();
	scope.activeTemplate = 'views/grantees-list.php';
	...
	scope.views.semesters = {
		...
		$http({
		  method: 'POST',
		  url: 'controllers/grantees.php?r=list'
	...
	this.view = function(scope,id) {
		...
		$http({
		  method: 'POST',
		  data: {id: id},
		  url: 'controllers/grantees.php?r=view'
		}).then(function mySucces(response) {
	...
	this.save = function(scope) {	
		...
		$http({
		  method: 'POST',
		  url: 'controllers/grantees.php?r='+scope.views.mode,
	...
	this.del = function(scope,id) {
		...
		function del(id) {
			...
			$http({
			  method: 'POST',
			  data: {id: [id]},
			  url: 'controllers/grantees.php?r=delete'
			}).then(function mySucces(response) {
...
$scope.scholarshipSave = function() {
	...
	$http({
	  ...
	  url: 'controllers/grantees.php?r=update_scholarship'
	}).then(function mySucces(response) {
